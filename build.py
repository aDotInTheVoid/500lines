...
def main(chapters=[], epub=False, pdf=False, html=False, mobi=False, pandoc_epub=False):
    ...
    if len(chapters) > 0:
        chapter_dirs = [
            chapter_dir for chapter_dir in chapter_dirs if chapter_dir in chapters
        ]
    ...
    process_chapters = [
        chapter_markdown
        for chapter_markdown, process in zip(chapter_markdowns, chapter_markdowns_exist)
        if process == 0
    ]
...
def build_epub(chapter_markdowns, pandoc_epub):
    basenames = [
        os.path.splitext(os.path.split(chapter_markdown)[1])[0] + ".markdown"
        for chapter_markdown in chapter_markdowns
    ]
...
def getbasename(chapter_markdown):
    ...
    basename = os.path.splitext(os.path.split(chapter_markdown)[1])[0]
    return basename
...
def preprocessor_command(chapter_markdown):
    temp = (
        "python _build/preprocessor.py --output=tex/{basename}.markdown --markdown {md}"
    )
    basename = getbasename(chapter_markdown)
