...
def remove_tags(tagname, text):
    ...
    """
    regex = re.compile(
        r"<" + tagname + r">(.|\s+)*?</" + tagname + r">", flags=re.MULTILINE
    )
...
""",
            [
                ("\\aosafigure[240pt]{bla}{bla}{bla}", "240pt", "bla", "bla", "bla"),
                ("\\aosafigure{bla}{bla}{bla}", "", "bla", "bla", "bla"),
            ],
        ),
    )
...
def fix_figure_index(text, chapter):
    ...
    for figure in figures_list:
        ...
        html_figure_text = (
            '<div class="center figure"><a name="figure-'
            + fignum
            + '"></a><img src="'
            + location
            + '" alt="Figure '
            ...
            + " - "
            + label
            + '" /></div><p class="center figcaption"><small>Figure '
            + fignum
            + " - "
            + label
            + "</small></p>"
...
def fix_table_references(text, chapter):
    ...
    table_caption_regex = re.compile(
        "^:\s\\\\label\{([a-zA-Z0-9\.\~\!\?]*)\}", re.MULTILINE
    )
    ...
    for table in tables:
        ...
        text = text.replace(
            "\\label{" + table + "}", "<b>Table " + table_id + "</b> - "
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Remove the specified kind of tag from the input document"
    )
